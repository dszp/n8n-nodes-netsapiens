...
- **Fixed the OAuth2 credential never fetching an access token when used outside this node.** n8n only invokes a credential's `preAuthentication` if the credential declares a hidden property with `typeOptions: { expirable: true }` — `CredentialsHelper.preAuthentication` returns early when none exists, with no error or warning. This credential had no such property, so `_accessToken` was never populated and the OAuth2 `authenticate` block resolved to a bare `Bearer `. The node itself was unaffected, because its transport layer sets the Authorization header directly, but selecting this credential in an HTTP Request node (Authentication → Predefined Credential Type) failed to authenticate under OAuth2. A hidden `_accessToken` property now backs the token, which n8n stores, reuses across requests, and refreshes on a 401. The API Key method was never affected, and existing credentials keep working without being re-entered.
